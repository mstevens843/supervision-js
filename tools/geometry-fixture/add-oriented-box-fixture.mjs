 ...
 * inference. It takes each frame's existing, already-committed SAM3-derived
 * `rect` for the "basketball" detection and rotates it by a fixed, hand-
 * chosen angle around its own center. The result is clearly a synthetic
 ...
 *
 * Frames where the ball's rect is implausibly large (a small number of
 * degenerate full-frame SAM3 boxes already present in the fixture) are left
...
const FIXTURE_DIR = join(REPO_ROOT, "demo", "fixtures", "basketball_sam3");
...
function main() {
  ...
  for (const fileName of chunkFiles) {
    ...
    for (const frame of chunk.frames) {
      const ball = frame.detections.find(
        (detection) => detection.className === "basketball",
      );
